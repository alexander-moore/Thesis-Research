## changing to 1/pi on pi-feature
...
for (i in 1:it) {
  ...
  x_train <- as.matrix(reduced_df_nolab)
  x_test <- as.matrix(dropped_obs_nolab)
  ...
  ham <- (1 / hat_N_sample)*(sum(reduced_df$FINCBTAX / reduced_df$pi) 
                             + sum(nn_pi_y_hat / dropped_obs$pi))
  
  statistic_tracker$nn_pi_imp_mean[i] <- ham
  ...
  x1pi <- (reduced_df_nolab$TOTEXPCQ)*(1 / (reduced_df_nolab$pi))
  x2pi <- (reduced_df_nolab$AGE_REF)*(1 / (reduced_df_nolab$pi))
  
  dx1pi <- (dropped_obs_nolab$TOTEXPCQ)*(1 / (dropped_obs_nolab$pi))
  dx2pi <- (dropped_obs_nolab$AGE_REF)*(1 / (dropped_obs_nolab$pi))
...
write.csv(statistic_tracker, file = "C:\\Users\\Alexander\\Documents\\thesis stat tracker\\1pi_2.csv")
