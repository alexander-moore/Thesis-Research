...
The fundamental statistical idea behind all of design-based inference is that an individual sampled with a sampling probability $\pi_i$ represents $\frac{1}{\pi_i}$ individuals in the population. The value $\frac{1}{\pi_i}$ is called the sampling weight [@lumley2011complex].
...
Data collected under a complex survey design have an additional layer of complexity and are not to be treated as typical independent and identically distributed (*i.i.d.*) data. Ignoring this complex survey design is found to create significant error in data analyses [@toth2011building]. This concern motivates our exploration of accounting for survey design in neural network imputation.

## Imputation

Often in real-world data, there is some degree of missingness. This can be for any number of reasons, illustrated in @missingness :

```{r tabel, echo=FALSE, cache=TRUE}
## Make table
library(kableExtra)
library(dplyr)
...
```{r missingness, echo=FALSE, cache=TRUE, warning = FALSE}
## Make table
library(kableExtra)
library(dplyr)
