...
for (i in 1:it) {
  ...
  #(1 / hat_N_sample) * sum((1 / df$pi) * df$y)
  
  #statistic_tracker$median_imp_mean[i] <- (1 / hat_N_sample) * sum((1 / reduced_df$pi) *imputed_list)
  ...
  weight_vec <- as.numeric(orig_df$pi)  
  ...
  # SHOULD THIS BE INVERSE OF THIS
  obs_weights <- partial_x_train[,3]  # this is non-generalizable and will be a problem if we change the number of features. 
  partial_x_train <- partial_x_train[,-3]
  x_train <- x_train[,-3]
  x_val <- x_val[,-3]
...
write.csv(statistic_tracker, file = "C:\\Users\\Alexander\\Documents\\thesis stat tracker\\test_5.csv")
