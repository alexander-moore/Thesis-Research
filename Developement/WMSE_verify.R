...
for (i in 1:it) {
  ...
  history <- model %>% fit(
    ...
    epochs = 100, #this is fishy ASSURE YOURSELF OF THIS
    verbose = 0,
    batch_size = 1,
    validation_data = list(x_val, y_val)
  )
  ...
  history <- model %>% fit(
    ...
    partial_y_train,
    sample_weight = obs_weights,
    epochs = goodtrain, #this is fishy ASSURE YOURSELF OF THIS
    verbose = 0,
    batch_size = 1,
    validation_data = list(x_val, y_val)
  )
...
write.csv(statistic_tracker, file = "C:\\Users\\Alexander\\Documents\\thesis stat tracker\\NO_oracle_1.csv")
