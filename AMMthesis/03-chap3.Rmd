...
Weighted Loss Neural Network Imputation takes inspiration from the weighted linear regression algorithm. This neural network training uses the same $\pi$-weighted MSE of equation \@ref(eq:wsme), but sacrifices the convexity of the loss function of the linear model, which means pervasive local minima which are distracting to the learning algorithm:
$$
...
```{r feature, echo = FALSE, out.width="50%"}
...
```{r bspic, echo = FALSE, out.width = "50%"}
